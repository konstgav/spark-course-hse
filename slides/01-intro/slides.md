...
---
## Как выставляется оценка по дисциплине

### **ROUND( 0.3 * ПР1 + 0.5 * ПР2 + 0.2 * Экз )**
1. **ПР1** - практическая работа от Алексея Юрьевича, до **10 баллов**
2. **ПР2** - практическая работа по миникурсу "Инженерная аналитика больших данных"
   - практическая самостоятельная работа, до **6 баллов**
   - посещение занятий, до **2 баллов**
   - работа на занятиях (ответы на вопросы, вовлеченность, проактивность), до **2 баллов**
3. **Экз** - оценка за экзамен, до **10 баллов**
   - задания по первой части курса
   - вопросы с выбором ответов по второй части курса
   - открытые вопросы по второй части курса
   - написать SQL (DDL, DML)
...
Каталог файлов в HDFS — ещё не таблица: нет **атомарности** и **истории**, **эволюция схемы**
ломает старые файлы, список файлов — медленный обход директорий.

**Apache Iceberg** хранит рядом с данными дерево метаданных, а каталог **Hive Metastore**
(его таблицы лежат в Postgres) — только **указатель** на корень этого дерева:

![w:1020](img/iceberg-hms.svg)

**Коммит** = новый `metadata.json` + атомарная замена `metadata_location`; старые снапшоты
остаются — можно читать «как было вчера».
...
| Инструмент | Что это и зачем нам |
|---|---|
| **Git** | контроль версий: `git clone` скачивает репозиторий курса, `git pull` — обновления |
| **Git Bash** | терминал с командами Linux (`cd`, `ls`) для Windows, ставится вместе с Git |
| **PowerShell** | стандартный терминал Windows: запускаем в нём установочные скрипты `.ps1` |
| **WSL 2** | «Linux внутри Windows» — лёгкая виртуальная машина, в ней работают контейнеры |
| **Docker Desktop** | запускает **контейнеры** — изолированные процессы со своим окружением |
| **Docker Compose** | стенд описан в `docker-compose.yml`, `docker compose up -d` поднимает его целиком |
| **conda** | менеджер окружений: Python 3.11 и Java 17 в окружении `spark-course`, отдельно от системных |
| **Java 17** | Spark работает в JVM; pyspark — обёртка, которая запускает JVM рядом с Python |
| **JupyterLab** | ноутбуки в браузере: код по ячейкам, результат — сразу под ячейкой |

HDFS, Spark, Hive Metastore, Airflow и Postgres — каждый в своём контейнере.
